...
static unsigned test_ro(void)
{
	...
	m = ioremap(ALGO_MEM_BASE, ALGO_MEM_SIZE);
	...
	for (i = 0; i < (1<<16); i++) {
		...
		if (v != e) {
			...
			printk(KERN_INFO "RO mismatch at 0x%08x, expected 0x%08x, got 0x%08x\n",
			       p - m + ALGO_MEM_BASE, (unsigned)e, (unsigned)v);
...
static unsigned test_rw(void)
{
	...
	m = ioremap(BLOCK_RAM_BASE, BLOCK_RAM_SIZE);

	for (i = 0; i < BLOCK_RAM_SIZE / 4; i++) {
		uint32_t o = i;
		char *p = m + o * 4;
	...
	for (i = 0; i < BLOCK_RAM_SIZE / 4; i++) {
		...
		if (v != e) {
			...
			printk(KERN_INFO "RW mismatch at 0x%08x, expected 0x%08x, got 0x%08x (0x%08x%s)\n",
			       p - m + BLOCK_RAM_BASE, (unsigned)e, (unsigned)v,
